...
// 查询剩余余额卷
$.post({url:"https://act.10010.com/SigninApp/new_convert/prizeList",headers:headerPrize,
        method: 'POST'},
    (err, resp, data) => {
        try {
            ...
            if (res.status !== '0000') {
                ...
                for (const tab of tabItems) {
                    ...
                    if (tab.state !== "已结束" || tab.state !== "即将开始") {
                        ...
                        for (const product of tab.timeLimitQuanListData) {
                            // 找到10元话费充值抵扣券
                            if (product.product_name === "3元话费充值抵扣券" && product.stockSurplus > 0) {
                ...
                if (targetProduct) {
                    $.log("* 找到目标商品: " + targetProduct.product_name);
...
// 生成抢购uuid
function generateUUID(productId) {
    $.post({url:"https://act.10010.com/SigninApp/convert/prizeConvert",
            headers: headerPrize,
...
function exchangePrize(uuid) {
    $.post({url:"https://act.10010.com/SigninApp/convert/prizeConvertResult",
            headers: headerPrize,
            body: "uuid=" + uuid,
            method: 'POST'},
        (err, resp, data) => {
...
function queryBalance() {
    $.post({url:"https://act.10010.com/SigninApp/convert/getTelephone",
            headers: headerPrize,
            method: 'POST'},
        (err, resp, data) => {
            try {
                ...
                if (res.status !== '0000') {
                    ...
                    $.log("* 余额查询成功: " + JSON.stringify(res.data));
                    $.msg("联通余额兑换", "兑换成功", "当前余额: " + res.data.telephone);
                }
            } catch (e) {
                $.log(e);
                $.msg("联通余额兑换失败", "详情", e);
